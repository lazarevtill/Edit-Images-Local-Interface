...
# SDNQ quantized models (complete pipelines, not GGUF)
SDNQ_MODELS = {
    "SDNQ-uint4-svd-r32 (17.3 GB)": "Disty0/Qwen-Image-Edit-2511-SDNQ-uint4-svd-r32",
}

# Combined model list for UI
AVAILABLE_MODELS = {**GGUF_MODELS, **SDNQ_MODELS}

# Lightning LoRA for fast 4-step inference (10x speedup)
LIGHTNING_LORA_REPO = "lightx2v/Qwen-Image-Edit-2511-Lightning"
LIGHTNING_LORA_FILES = {
    "4-step (BF16)": "Qwen-Image-Edit-2511-Lightning-4steps-V1.0-bf16.safetensors",
    "4-step (FP32)": "Qwen-Image-Edit-2511-Lightning-4steps-V1.0-fp32.safetensors",
}
LIGHTNING_LORA_DIR = "./models/lightning-lora"
...
SDNQ_PIPELINE_DIR = "./models/Qwen-Image-Edit-2511-SDNQ-uint4"

# Global pipeline cache
current_pipeline = None
current_model_name = None
current_lora_loaded = None  # Track if Lightning LoRA is loaded
current_compiled = False  # Track if torch.compile is applied
current_device = None
...
def is_sdnq_model(model_name: str) -> bool:
    """Check if a model is an SDNQ model (complete pipeline) vs GGUF."""
    return model_name in SDNQ_MODELS


def get_sdnq_local_path(model_name: str) -> Path:
    """Get local path for SDNQ model."""
    # Use a simplified folder name
    return Path(SDNQ_PIPELINE_DIR)
...
def get_downloaded_models() -> List[str]:
    ...
    # Check GGUF models
    for model_name, filename in GGUF_MODELS.items():
        if (models_path / filename).exists():
            downloaded.append(model_name)

    # Check SDNQ models
    for model_name in SDNQ_MODELS.keys():
        sdnq_path = get_sdnq_local_path(model_name)
        if sdnq_path.exists() and (sdnq_path / "model_index.json").exists():
            downloaded.append(model_name)
...
def check_model_status(model_name: str) -> str:
    ...
    if is_sdnq_model(model_name):
        # SDNQ model - check for complete pipeline directory
        sdnq_path = get_sdnq_local_path(model_name)
        if sdnq_path.exists() and (sdnq_path / "model_index.json").exists():
            # Calculate total size
            total_size = sum(f.stat().st_size for f in sdnq_path.rglob("*") if f.is_file())
            size_gb = total_size / (1024**3)
            return f"Downloaded ({size_gb:.2f} GB)"
        else:
            return "Not downloaded"
    else:
        # GGUF model - check for single file
...
def download_model(model_name: str, progress=gr.Progress()) -> str:
    ...
    if is_sdnq_model(model_name):
        # SDNQ model - download complete pipeline
        from huggingface_hub import snapshot_download

        repo_id = SDNQ_MODELS[model_name]
        sdnq_path = get_sdnq_local_path(model_name)

        if sdnq_path.exists() and (sdnq_path / "model_index.json").exists():
            total_size = sum(f.stat().st_size for f in sdnq_path.rglob("*") if f.is_file())
            size_gb = total_size / (1024**3)
            return f"Model already downloaded: {model_name} ({size_gb:.2f} GB)"

        try:
            progress(0, desc=f"Downloading {model_name} (this may take a while)...")

            snapshot_download(
                repo_id=repo_id,
                local_dir=str(sdnq_path),
            )

            total_size = sum(f.stat().st_size for f in sdnq_path.rglob("*") if f.is_file())
            size_gb = total_size / (1024**3)
            return f"Successfully downloaded: {model_name} ({size_gb:.2f} GB)"

        except Exception as e:
            return f"Error downloading {model_name}: {str(e)}"
    else:
        # GGUF model - download single file
        filename = AVAILABLE_MODELS[model_name]
...
def delete_model(model_name: str) -> str:
    ...
    try:
        ...
        if is_sdnq_model(model_name):
            # SDNQ model - delete entire directory
            sdnq_path = get_sdnq_local_path(model_name)
            if not sdnq_path.exists():
                return f"Model not found: {model_name}"
            shutil.rmtree(sdnq_path)
            return f"Successfully deleted: {model_name}"
        else:
            # GGUF model - delete single file
            filename = AVAILABLE_MODELS[model_name]
            local_path = Path(MODELS_DIR) / filename
...
def get_all_model_status() -> str:
    ...
    # Check base pipeline (for GGUF models)
    ...
    if base_pipeline_path.exists() and (base_pipeline_path / "model_index.json").exists():
        ...
        lines.append(f"- **Base Pipeline** (for GGUF): {base_size:.2f} GB")
        total_size += base_size
    else:
        lines.append("- **Base Pipeline** (for GGUF): *Not downloaded*")

    lines.append("")  # Empty line separator

    # Check SDNQ models
    lines.append("### SDNQ Models")
    sdnq_count = 0
    for model_name in SDNQ_MODELS.keys():
        sdnq_path = get_sdnq_local_path(model_name)
        if sdnq_path.exists() and (sdnq_path / "model_index.json").exists():
            size_gb = sum(f.stat().st_size for f in sdnq_path.rglob("*") if f.is_file()) / (1024**3)
            total_size += size_gb
            downloaded_count += 1
            sdnq_count += 1
    ...
    if sdnq_count == 0:
        lines.append("*No SDNQ models downloaded*")
    ...
    if gguf_count == 0:
        lines.append("*No GGUF models downloaded*")
...
def get_model_path(model_filename: str, models_dir: str = "./models") -> str:
    ...
    print(f"Downloading {model_filename} from {GGUF_REPO_ID}...")
    downloaded_path = hf_hub_download(
        repo_id=GGUF_REPO_ID,
...
def download_lightning_lora(lora_name: str = "4-step (BF16)", progress=gr.Progress()) -> str:
    """Download Lightning LoRA for fast 4-step inference."""
    if lora_name not in LIGHTNING_LORA_FILES:
        return f"Error: Unknown LoRA {lora_name}"

    filename = LIGHTNING_LORA_FILES[lora_name]
    lora_path = Path(LIGHTNING_LORA_DIR)
    lora_path.mkdir(parents=True, exist_ok=True)

    local_path = lora_path / filename

    if local_path.exists():
        size_mb = local_path.stat().st_size / (1024**2)
        return f"Lightning LoRA already downloaded: {filename} ({size_mb:.1f} MB)"

    try:
        progress(0, desc=f"Downloading Lightning LoRA {filename}...")

        downloaded_path = hf_hub_download(
            repo_id=LIGHTNING_LORA_REPO,
            filename=filename,
            local_dir=LIGHTNING_LORA_DIR,
        )

        size_mb = Path(downloaded_path).stat().st_size / (1024**2)
        return f"Successfully downloaded: {filename} ({size_mb:.1f} MB)"

    except Exception as e:
        return f"Error downloading Lightning LoRA: {str(e)}"


def check_lightning_lora_status() -> str:
    """Check if Lightning LoRA is downloaded."""
    lora_path = Path(LIGHTNING_LORA_DIR)
    downloaded = []

    for name, filename in LIGHTNING_LORA_FILES.items():
        if (lora_path / filename).exists():
            size_mb = (lora_path / filename).stat().st_size / (1024**2)
            downloaded.append(f"- {name}: {size_mb:.1f} MB")

    if downloaded:
        return "**Downloaded Lightning LoRAs:**\n" + "\n".join(downloaded)
    else:
        return "*No Lightning LoRAs downloaded*"


def apply_optimizations(pipeline, device_str: str, optimizations: Dict[str, bool]):
    """Apply various optimizations to the pipeline."""
    applied = []

    # VAE Tiling - for large images, reduces memory
    if optimizations.get("vae_tiling", False):
        try:
            pipeline.enable_vae_tiling()
            applied.append("VAE tiling")
        except Exception as e:
            print(f"Could not enable VAE tiling: {e}")

    # VAE Slicing - for batch processing
    if optimizations.get("vae_slicing", False):
        try:
            pipeline.enable_vae_slicing()
            applied.append("VAE slicing")
        except Exception as e:
    ...
    if optimizations.get("attention_slicing", False):
        try:
            ...
            applied.append("Attention slicing")
        except Exception as e:
            print(f"Could not enable attention slicing: {e}")

    # Channels last memory format - can improve speed on some GPUs
    if optimizations.get("channels_last", False) and "cuda" in device_str:
        try:
            pipeline.unet = pipeline.unet.to(memory_format=torch.channels_last) if hasattr(pipeline, 'unet') else None
            if hasattr(pipeline, 'transformer'):
                pipeline.transformer = pipeline.transformer.to(memory_format=torch.channels_last)
            applied.append("Channels last")
        except Exception as e:
            print(f"Could not enable channels last: {e}")

    # xFormers memory efficient attention (if available)
    if optimizations.get("xformers", False):
        try:
...
def apply_torch_compile(pipeline, device_str: str):
    """Apply torch.compile to the transformer for faster inference."""
    global current_compiled

    if current_compiled:
        print("torch.compile already applied")
        return pipeline

    if "cuda" not in device_str:
        print("torch.compile only supported on CUDA")
        return pipeline
    ...
    try:
        ...
        # Configure inductor for best performance
        torch._inductor.config.conv_1x1_as_mm = True
        torch._inductor.config.coordinate_descent_tuning = True
        torch._inductor.config.epilogue_fusion = False
        torch._inductor.config.coordinate_descent_check_all_directions = True

        # Compile the transformer (main compute component)
...
def load_lightning_lora(pipeline, lora_type: str = "4-step (BF16)"):
    """Load Lightning LoRA for fast 4-step inference."""
    global current_lora_loaded

    if lora_type == "None" or not lora_type:
        if current_lora_loaded:
            try:
                pipeline.unload_lora_weights()
                current_lora_loaded = None
                print("Unloaded Lightning LoRA")
            except:
                pass
        return pipeline, 40  # Default steps

    if current_lora_loaded == lora_type:
        print(f"Lightning LoRA {lora_type} already loaded")
        return pipeline, 4  # 4 steps for Lightning

    filename = LIGHTNING_LORA_FILES.get(lora_type)
    if not filename:
        print(f"Unknown LoRA type: {lora_type}")
        return pipeline, 40

    lora_path = Path(LIGHTNING_LORA_DIR) / filename

    if not lora_path.exists():
        print(f"Lightning LoRA not downloaded: {filename}")
        print("Download it from the Model Manager tab first")
        return pipeline, 40
    ...
    try:
        ...
        print(f"Loading Lightning LoRA: {filename}")
        pipeline.load_lora_weights(str(lora_path))
        current_lora_loaded = lora_type
        print(f"Lightning LoRA loaded - using 4 steps for inference")

        return pipeline, 4  # Lightning uses 4 steps

    except Exception as e:
        print(f"Could not load Lightning LoRA: {e}")
        return pipeline, 40
...
def load_pipeline(model_name: str, device_choice: str):
    """Load the pipeline with the specified model and device."""
    global current_pipeline, current_model_name, current_device
    ...
    if is_sdnq_model(model_name):
        # SDNQ model - load complete pipeline directly
        print("Loading SDNQ quantized pipeline...")

        sdnq_path = get_sdnq_local_path(model_name)
        repo_id = SDNQ_MODELS[model_name]

        # Use local path if downloaded, otherwise download from HF
        if sdnq_path.exists() and (sdnq_path / "model_index.json").exists():
            pipeline_source = str(sdnq_path)
            print(f"Using local SDNQ pipeline: {pipeline_source}")
        else:
            pipeline_source = repo_id
            print(f"Using HuggingFace SDNQ pipeline: {pipeline_source}")
        ...
        for attempt in range(2):
            try:
                ...
                pipeline = QwenImageEditPlusPipeline.from_pretrained(
                    ...
                    force_download=force_download if pipeline_source == repo_id else False,
                )
                break
        ...
        # GGUF model - load transformer separately with GGUF quantization
        from diffusers import QwenImageTransformer2DModel, GGUFQuantizationConfig

        model_filename = GGUF_MODELS[model_name]
    ...
    if device_str == "cpu":
        ...
        # NVIDIA CUDA - move pipeline to GPU
...
def infer(
    ...
    try:
        ...
        optimizations = {
            ...
            "attention_slicing": True,  # Always enable for memory efficiency
        }
        pipe, applied_opts = apply_optimizations(pipe, device_str, optimizations)

        # Apply torch.compile if requested (CUDA only)
        if use_torch_compile and "cuda" in device_str:
            pipe = apply_torch_compile(pipe, device_str)

        # Load Lightning LoRA if requested (10x speedup with 4 steps)
        actual_steps = num_inference_steps
        if use_lightning_lora and use_lightning_lora != "None":
            pipe, actual_steps = load_lightning_lora(pipe, use_lightning_lora)
            # Lightning uses reduced guidance scale
            if actual_steps == 4:
                true_guidance_scale = min(true_guidance_scale, 1.0)
                print(f"Lightning mode: using {actual_steps} steps, guidance scale {true_guidance_scale}")
        ...
        print(f"Device: {device_choice}, Seed: {seed}, Steps: {actual_steps}, Guidance: {true_guidance_scale}")
        if applied_opts:
            print(f"Optimizations: {', '.join(applied_opts)}")
        ...
        opt_str = f", Opts: {', '.join(applied_opts)}" if applied_opts else ""
        lightning_str = f" (Lightning {use_lightning_lora})" if use_lightning_lora != "None" else ""
        status_msg = f"Generation complete! Device: {device_choice}, Seed: {seed}, Steps: {actual_steps}{lightning_str}{opt_str}\nSaved {len(output_paths)} output images to {OUTPUT_IMAGES_DIR}"
        ...
        with gr.Column(elem_id="col-container"):
            ...
            with gr.Tabs():
                ...
                with gr.TabItem("Image Editing"):
                    ...
                    # Speed Optimizations
                    with gr.Accordion("Speed & Memory Optimizations", open=True):
                        gr.Markdown("**Lightning Mode** - 10x faster with 4 steps (download LoRA in Model Manager first)")
                        with gr.Row():
                            lightning_lora = gr.Dropdown(
                                choices=["None"] + list(LIGHTNING_LORA_FILES.keys()),
                                value="None",
                                label="Lightning LoRA",
                                info="Enable for ~10x speedup (4 steps instead of 40)"
                            )
                            torch_compile = gr.Checkbox(
                                label="torch.compile",
                                value=False,
                                info="CUDA only, slow first run, ~20% faster after"
                            )

                        gr.Markdown("**Memory Optimizations** - Enable if you have limited VRAM (8-12GB)")
                        with gr.Row():
                            vae_tiling = gr.Checkbox(
                                label="VAE Tiling",
                                value=False,
                                info="For large images, reduces peak VRAM"
                            )
                            vae_slicing = gr.Checkbox(
                                label="VAE Slicing",
                                value=False,
                                info="For batch generation, process one at a time"
                            )
                            xformers = gr.Checkbox(
                                label="xFormers",
                                value=False,
                                info="Memory-efficient attention (if installed)"
                            )
                    ...
                    with gr.Accordion("Generation Settings", open=False):
                        ...
                        with gr.Row():
                            true_guidance_scale = gr.Slider(
                                ...
                                step=0.1,
                                value=4.0,
                                info="Higher values follow the prompt more strictly (use 1.0 for Lightning)"
                            )
                            num_inference_steps = gr.Slider(
                                ...
                                step=1,
                                value=40,
                                info="40 normal, 4 with Lightning LoRA"
                            )
                ...
                with gr.TabItem("Model Manager"):
                    ...
                    # Lightning LoRA Section
                    gr.Markdown("---")
                    gr.Markdown("## Lightning LoRA (Speed Boost)")
                    gr.Markdown(
                        "Download Lightning LoRA for **10x faster inference** (4 steps instead of 40). "
                        "Works with any base model above."
                    )

                    with gr.Row():
                        with gr.Column(scale=1):
                            lightning_lora_dropdown = gr.Dropdown(
                                choices=list(LIGHTNING_LORA_FILES.keys()),
                                value="4-step (BF16)",
                                label="Select Lightning LoRA",
                                info="BF16 recommended for most GPUs"
                            )
                            download_lora_button = gr.Button(
                                "Download Lightning LoRA",
                                variant="primary",
                            )
                            lora_download_status = gr.Textbox(
                                label="LoRA Download Status",
                                interactive=False,
                                lines=2,
                            )

                        with gr.Column(scale=1):
                            lightning_status_md = gr.Markdown(
                                value=check_lightning_lora_status(),
                                label="Lightning LoRA Status",
                            )
                    ...
                    ### SDNQ Models (Alternative Quantization)

                    | Model | Size | Quality | Notes |
                    |-------|------|---------|-------|
                    | **SDNQ-uint4-svd-r32** | **17.3 GB** | **Good** | 4-bit with SVD, complete pipeline |
                    """)
                ...
                with gr.TabItem("Help"):
                    ...
                    - **Multiple Quantizations**: GGUF, SDNQ, and full precision options
                    - **Multi-Device Support**: NVIDIA CUDA, Intel XPU, and CPU
                    - **Speed Optimizations**: Lightning LoRA (10x faster), torch.compile, VAE optimizations
                    ...
                    | VRAM | Recommended Setup | Expected Speed |
                    |------|-------------------|----------------|
                    | 8 GB | Q2_K/Q3_K + Lightning LoRA + VAE Tiling | ~30s/image |
                    | 12 GB | Q4_K_M + Lightning LoRA | ~15s/image |
                    | 24 GB | Q6_K/Q8_0 + Lightning LoRA + torch.compile | ~5s/image |

                    **Lightning LoRA** - The biggest speedup! Reduces steps from 40 to 4 (10x faster)
                    1. Go to Model Manager tab
                    2. Download "4-step (BF16)" Lightning LoRA
                    3. Select it in "Speed & Memory Optimizations" section
                    4. Use guidance scale 1.0 for best results

                    **torch.compile** - Additional 20% speedup (CUDA only)
                    - First run is slow (compilation)
                    - Subsequent runs are faster
                    - Best combined with Lightning LoRA

                    **VAE Tiling** - For large images or low VRAM
                    - Enables processing large images without OOM
                    - Slight quality reduction at tile boundaries

                    ### Tips for Best Results
                    1. **Be specific**: "Add a red hat to the person" works better than "add hat"
                    2. **Standard mode**: Use guidance scale 4-6, 40 steps
                    3. **Lightning mode**: Use guidance scale 1.0, 4 steps (automatic)
                    4. **Start with Q4_K_M**: Best balance of quality and speed

                    ### Troubleshooting
                    - **Out of memory**: Enable VAE Tiling, try smaller model (Q3_K_M, Q2_K)
                    - **Slow generation**: Enable Lightning LoRA (10x speedup!)
                    - **Poor quality**: Disable Lightning, increase guidance scale, use larger model
                    - **torch.compile errors**: Disable it, requires PyTorch 2.0+ and CUDA
                    ...
                    - [SDNQ Model](https://huggingface.co/Disty0/Qwen-Image-Edit-2511-SDNQ-uint4-svd-r32)
            ...
            # Event handlers for Lightning LoRA
            download_lora_button.click(
                fn=download_lightning_lora,
                inputs=[lightning_lora_dropdown],
                outputs=[lora_download_status],
            ).then(
                fn=check_lightning_lora_status,
                outputs=[lightning_status_md],
            )
